...
export default async function ProviderAcquisitionPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col"><Nav /><main className="flex-1 py-10"><div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className="eyebrow">Internal provider acquisition</div><h1 className="mt-2 font-display text-4xl text-parchment">Provider supply funnel</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-parchment-dim">One source of truth across markets for who can be contacted, who received an invitation, who opened it, who claimed, who completed secure payouts and who is available for work.</p></div><Link href="/admin" className="text-sm font-semibold text-brass">Network operations →</Link></div>
      ...
      <section className="mt-10"><div className="flex items-end justify-between"><div><div className="eyebrow">Provider-by-provider status</div><h2 className="mt-2 font-display text-3xl text-parchment">From public profile to active supply.</h2></div><div className="font-mono text-xs text-parchment-dim">Available now: {available.length}</div></div>
        <div className="mt-5 overflow-x-auto rounded-2xl border border-line bg-surface"><table className="min-w-full text-left text-sm"><thead className="border-b border-line bg-surface-raised"><tr className="font-mono text-[10px] uppercase tracking-[.1em] text-parchment-dim"><th className="px-4 py-3">Business</th><th className="px-4 py-3">Contact</th><th className="px-4 py-3">Invite</th><th className="px-4 py-3">Claim</th><th className="px-4 py-3">Payout</th><th className="px-4 py-3">Availability</th></tr></thead><tbody className="divide-y divide-line">
          {providers.map((provider: any) => {
            ...
            return <tr key={provider.id}><td className="px-4 py-4"><div className="font-medium text-parchment">{provider.business_name}</div><div className="mt-1 text-xs text-parchment-dim">{[provider.city,provider.state].filter(Boolean).join(", ") || "Location not listed"}</div></td><td className="px-4 py-4 text-xs text-parchment-dim">{email ? <><div>{email.contact_value}</div><div className="mt-1">{email.verification_status}{email.can_outreach ? " · outreach-ready" : ""}</div></> : <><div>No verified email</div><div className="mt-1">{source?.website || source?.phone || "Needs contact enrichment"}</div></>}</td><td className="px-4 py-4"><Badge value={invite?.status ?? "not invited"} /></td><td className="px-4 py-4"><Badge value={provider.claim_status} good={provider.claim_status === "verified"} /></td><td className="px-4 py-4"><Badge value={provider.stripe_payouts_enabled ? "ready" : provider.stripe_account_id ? "in progress" : "not started"} good={provider.stripe_payouts_enabled} /></td><td className="px-4 py-4"><Badge value={provider.is_available && provider.stripe_payouts_enabled ? "available" : "offline"} good={provider.is_available && provider.stripe_payouts_enabled} /></td></tr>;
          })}
        </tbody></table></div>
      </section>

      <section className="mt-8 rounded-2xl border border-line bg-surface p-6"><div className="eyebrow">Contact-data gap</div><h2 className="mt-2 font-display text-2xl text-parchment">Only verified outreach-eligible contacts enter automated invitations.</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-parchment-dim">The current Boston seed contains public phone numbers and some websites but not verified business emails, and New York expansion contacts also need verified email enrichment before automated outreach. The system does not treat a phone number as permission for automated SMS or calling.</p><p className="mt-3 text-xs leading-5 text-brass">REQUIRES QUALIFIED LEGAL REVIEW before activating outbound campaign sending.</p></section>
    </div></main><Footer /></div>
  );
}
