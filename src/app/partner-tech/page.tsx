...
const MARKETS = [
  ["Boston & Greater Boston", "Primary launch market. Providers select only the Massachusetts locations they genuinely cover."],
  ["New York", "Expansion market covering New York City, selected neighborhoods and selected New York State markets."],
  ["New Jersey & Philadelphia", "Northeast expansion markets with hyperlocal city and neighborhood discovery pages. Geographic selection does not replace state or local provider-eligibility requirements."],
  ["Connecticut & Delaware", "Expansion markets with selected urban, suburban and coastal service areas. Providers choose only their real operating radius; credential requirements remain a separate activation gate."],
] as const;
...
export default function PartnerTechPage() {
  return (
    <div className="flex min-h-screen flex-col"><Nav /><main className="flex-1">
      ...
      <section className="border-b border-line/60 py-14 sm:py-16"><div className="mx-auto max-w-[1180px] px-6 sm:px-8 lg:px-10"><div className="eyebrow">Market status</div><div className="mt-6 grid gap-5 md:grid-cols-2">{MARKETS.map(([title, body]) => <div key={title} className="border-t border-line pt-5"><h2 className="font-display text-2xl text-parchment">{title}</h2><p className="mt-3 text-sm leading-6 text-parchment-dim">{body}</p></div>)}</div><p className="mt-6 max-w-3xl text-xs leading-5 text-parchment-dim">Being listed on a geographic page or selecting a service area does not by itself establish licensing, registration, insurance or other provider eligibility. Those requirements must be satisfied separately where applicable before paid activation.</p></div></section>
