...
export default async function ClaimProviderPage({ searchParams }: { searchParams: Promise<{ provider?: string; invite?: string; error?: string }> }) {
  ...
  if (invite && hasAdminClientConfig()) {
    ...
    const { data: invitation } = await admin.from("provider_outreach_invites").select("provider_id,contact_id,status,expires_at").eq("token_hash", hashProviderInviteToken(invite)).maybeSingle();
  ...
  const { data: providers } = await supabase.from("provider_profiles").select("id,slug,business_name,city,state,claim_status").order("business_name");
  const provider = providerParam ? (providers ?? []).find((item: any) => item.id === providerParam || item.slug === providerParam) : null;
  ...
  if (!provider) {
    return (
      <div className="flex min-h-screen flex-col"><Nav /><main className="flex-1 py-16"><div className="mx-auto max-w-4xl px-6">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-6 text-parchment-dim">Profiles can be prepared from public business information so local locksmith businesses can claim and complete onboarding without rebuilding their listing from scratch.</p>
        <div className="mt-8 divide-y divide-line rounded-2xl border border-line bg-surface">
          {(providers ?? []).map((item: any) => <Link key={item.id} href={`/providers/claim?provider=${item.id}`} className="flex items-center justify-between gap-4 px-5 py-4 hover:bg-surface-raised"><span><span className="block text-sm font-medium text-parchment">{item.business_name}</span><span className="mt-1 block text-xs text-parchment-dim">{[item.city, item.state].filter(Boolean).join(", ") || "Location not listed"} · {item.claim_status.replaceAll("_", " ")}</span></span><span className="text-brass">Claim →</span></Link>)}
        </div>
  ...
  const { data: existingClaim } = user ? await supabase.from("provider_claims").select("id,status").eq("provider_id", provider.id).eq("user_id", user.id).maybeSingle() : { data: null };
  ...
  return (
    <div className="flex min-h-screen flex-col"><Nav /><main className="flex-1 py-16"><div className="mx-auto max-w-2xl px-6">
      ...
      <p className="mt-2 text-sm text-parchment-dim">{[provider.city, provider.state].filter(Boolean).join(", ") || "Location not listed"}</p>
      ...
      {existingClaim?.status === "approved" ? (
        ...
        <form action={claimProvider} className="mt-8 space-y-4 rounded-2xl border border-line bg-surface p-6">
          <input type="hidden" name="provider_id" value={provider.id} /><input type="hidden" name="invite_token" value={invite ?? ""} />
