...
export default async function ProviderProfilePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const claimVerified = provider.claim_status === "verified";
      ...
      <main className="flex-1 py-16">
        <div className="mx-auto max-w-3xl px-6">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <span className={`rounded-full px-3 py-1 font-mono text-[10px] uppercase tracking-wide ${claimVerified ? "bg-verdigris/10 text-verdigris" : "bg-brass/10 text-brass"}`}>
              {claimVerified ? "Business claim verified" : "Unclaimed profile"}
            </span>
            {provider.is_available && claimVerified ? <span className="rounded-full bg-verdigris/10 px-3 py-1 font-mono text-[10px] uppercase tracking-wide text-verdigris">Availability on</span> : null}
          </div>
          ...
          <p className="mt-6 text-sm leading-7 text-parchment-dim">
            {provider.description || (claimVerified
              ? "This business has claimed its Trusted Locksmith provider profile, and its connection to the business has been verified."
          ...
          <div className="mt-8 rounded-2xl border border-line bg-surface p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-parchment-dim">
              {claimVerified
                ? "A verified business claim does not by itself guarantee marketplace activation. Job offers are enabled only after any additional provider requirements for the service area are satisfied and availability is turned on."
