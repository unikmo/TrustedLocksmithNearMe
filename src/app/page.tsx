...
const MARKETS = [
  {
    title: "Massachusetts",
    body: "Boston and Greater Boston remain the primary launch market, with localized pages across the surrounding communities.",
    primaryLabel: "Boston",
    primaryHref: "/boston-ma",
    links: [["Cambridge", "/cambridge-ma"],["Newton", "/newton-ma"],["Somerville", "/somerville-ma"]],
  },
  {
    title: "New York",
    body: "Choose New York City, a borough, selected NYC neighborhoods or New York State markets. Availability remains address-specific.",
    primaryLabel: "New York City",
    primaryHref: "/new-york-ny",
    links: [["Manhattan", "/manhattan-ny"],["Brooklyn", "/brooklyn-ny"],["Queens", "/queens-ny"]],
  },
  {
    title: "New Jersey",
    body: "Hyperlocal pages now cover the strongest New Jersey markets from Hudson County through Central and South Jersey.",
    primaryLabel: "Jersey City",
    primaryHref: "/jersey-city-nj",
    links: [["Newark", "/newark-nj"],["Hoboken", "/hoboken-nj"],["Cherry Hill", "/cherry-hill-nj"]],
  },
  {
    title: "Philadelphia",
    body: "Philadelphia is treated as a set of real local markets rather than one generic city page.",
    primaryLabel: "Philadelphia",
    primaryHref: "/philadelphia-pa",
    links: [["West Philadelphia", "/west-philadelphia-pa"],["South Philadelphia", "/south-philadelphia-pa"],["Center City", "/center-city-philadelphia-pa"]],
  },
  {
    title: "Connecticut",
    body: "Local pages follow the I-95 and inland Connecticut markets with property, neighborhood and access context.",
    primaryLabel: "Stamford",
    primaryHref: "/stamford-ct",
    links: [["New Haven", "/new-haven-ct"],["Hartford", "/hartford-ct"],["Bridgeport", "/bridgeport-ct"]],
  },
  {
    title: "Delaware",
    body: "Delaware coverage connects Wilmington and Newark with Dover and the coastal second-home markets.",
    primaryLabel: "Wilmington",
    primaryHref: "/wilmington-de",
    links: [["Newark", "/newark-de"],["Dover", "/dover-de"],["Rehoboth Beach", "/rehoboth-beach-de"]],
  },
] as const;
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
          ...
          <div className="mx-auto grid max-w-[1180px] gap-8 px-6 py-10 sm:px-8 sm:py-14 lg:grid-cols-[.92fr_1.08fr] lg:items-center lg:gap-12 lg:px-10 lg:py-16">
            <div className="max-w-xl">
              ...
              <div className="mt-8 grid gap-3 border-t border-line/65 pt-6 sm:grid-cols-3">
                {[["Price first", "Know the standard total before you request."],["Real acceptance", "Provider details appear after acceptance."],["You approve extras", "No additional work starts without approval."]].map(([title, body]) => (
            ...
            <figure className="relative h-[390px] overflow-hidden rounded-[30px] border border-sky/18 bg-surface-raised shadow-[0_28px_70px_rgba(3,18,37,0.26)] sm:h-[500px]">
              ...
              <figcaption className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-void/95 via-void/55 to-transparent px-6 pb-6 pt-20">
                ...
                <p className="mt-2 max-w-md text-sm leading-6 text-parchment-dim">Trusted Locksmith operates the request platform. Field work is performed by participating independent local providers.</p>
              </figcaption>
            </figure>
          </div>
        </section>
        ...
        <section id="locations" className="border-b border-[#c7d9ec] bg-mist py-12 text-navy-text sm:py-14">
          <div className="mx-auto max-w-[1180px] px-6 sm:px-8 lg:px-10">
            <div className="max-w-2xl">
              ...
              <p className="mt-4 text-sm leading-6 text-[#536e8a]">Location pages explain local access conditions and route you toward the right request path. They do not imply that a provider is available at every address or hour.</p>
            </div>
          ...
          <div className="mx-auto max-w-[1180px] px-6 sm:px-8 lg:px-10">
            ...
            <div className="mt-9 divide-y divide-line/70 border-y border-line/70">
              {featuredServices.map((service) => (
                <Link key={service.id} href={`/book/details?service_id=${service.id}`} className="group grid gap-3 py-5 transition sm:grid-cols-[1fr_auto] sm:items-center">
        ...
        <section id="how-it-works" className="border-b border-line/60 bg-surface/38 py-16 sm:py-20">
          <div className="mx-auto max-w-[1180px] px-6 sm:px-8 lg:px-10">
            <div className="max-w-2xl"><div className="eyebrow">How it works</div><h2 className="mt-4 font-display text-4xl tracking-[-.03em] text-parchment sm:text-5xl">Three steps. No fictional dispatch.</h2></div>
        ...
        <section className="border-b border-line/60 py-14 sm:py-16">
          <div className="mx-auto grid max-w-[1180px] gap-10 px-6 sm:px-8 lg:grid-cols-[.82fr_1.18fr] lg:items-start lg:px-10">
            <div className="max-w-md"><div className="eyebrow">Why the process is different</div><h2 className="mt-4 font-display text-4xl tracking-[-.03em] text-parchment">Clarity before urgency takes over.</h2></div>
            <div className="divide-y divide-line/70 border-y border-line/70">
              {[["Standard price before the request", "You can see the published total and scope before a provider is involved."],["Provider identity after acceptance", "We do not show a made-up provider, rating or ETA while a request is still unmatched."],["Extra work stays your decision", "If the actual job is outside the standard scope, the additional work and price must be approved first."]].map(([title, body]) => <div key={title} className="py-5"><h3 className="font-semibold text-parchment">{title}</h3><p className="mt-1 text-sm leading-6 text-parchment-dim">{body}</p></div>)}
            </div>
          </div>
        </section>
        ...
        <section className="py-16 sm:py-20">
          <div className="mx-auto grid max-w-[1180px] gap-4 px-6 sm:px-8 md:grid-cols-2 lg:px-10">
            <div className="rounded-[28px] border border-brass/25 bg-brass/[.07] p-7 sm:p-8"><div className="eyebrow">Need a locksmith?</div><h2 className="mt-3 font-display text-3xl text-parchment sm:text-4xl">Start with the price, not a sales call.</h2><p className="mt-4 max-w-md text-sm leading-6 text-parchment-dim">Choose the service and review the standard total before sending the request.</p><Link href="/book" className="mt-6 inline-flex min-h-11 items-center rounded-full bg-brass px-6 py-2.5 text-sm font-semibold text-ink">Get my price →</Link></div>
            <div className="rounded-[28px] border border-line bg-surface/45 p-7 sm:p-8"><div className="eyebrow">Local locksmith businesses</div><h2 className="mt-3 font-display text-3xl text-parchment sm:text-4xl">Want local requests you can choose to accept?</h2><p className="mt-4 max-w-md text-sm leading-6 text-parchment-dim">Control your availability and review each request and its private commercial terms before deciding.</p><Link href="/partner-tech" className="mt-6 inline-flex min-h-11 items-center rounded-full border border-sky/25 px-6 py-2.5 text-sm font-semibold text-parchment transition hover:border-sky/50">For locksmiths →</Link></div>
          </div>
        </section>
      </main>
      <Footer />
