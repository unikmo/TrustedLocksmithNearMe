...
export function LocalAreaHero({
  ...
  return (
    <section className="border-b border-line/60">
      <div className="mx-auto grid max-w-[1180px] gap-10 px-6 py-10 sm:px-8 sm:py-14 lg:grid-cols-[.92fr_1.08fr] lg:items-center lg:gap-12 lg:px-10 lg:py-16">
        <div className="max-w-xl">
          <div className="font-mono text-[11px] uppercase tracking-[.14em] text-brass">{eyebrow}</div>
          <h1 className="mt-5 font-display text-5xl font-medium leading-[.96] tracking-[-.04em] text-parchment sm:text-6xl lg:text-[64px]">
            {copy.lead}
            <span className="block italic text-brass">{copy.accent}</span>
          </h1>
          <p className="mt-6 max-w-lg text-lg leading-8 text-parchment-dim">{copy.deck}</p>
          ...
          <div className="mt-6 rounded-2xl border border-brass/20 bg-brass/[.06] px-4 py-3 text-sm leading-6 text-parchment">
            <span className="font-semibold text-brass">Local note.</span> {copy.smile}
          </div>

          <div className="mt-7 grid gap-3 border-t border-line/65 pt-5 sm:grid-cols-3">
            {[
              ...
              <div key={title}>
                ...
                <div className="mt-1 text-xs leading-5 text-parchment-dim">{body}</div>
              </div>
            ))}
          </div>

          <p className="mt-5 max-w-lg text-xs leading-5 text-parchment-dim">{disclaimer}</p>
        </div>

        <figure>
          <div className="relative h-[360px] overflow-hidden rounded-[30px] border border-sky/18 bg-surface-raised shadow-[0_28px_70px_rgba(3,18,37,0.26)] sm:h-[470px]">
            <Image
              src={heroImage.src}
              alt={heroImage.alt}
              fill
            ...
            />
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-void/95 via-void/50 to-transparent px-6 pb-6 pt-24">
