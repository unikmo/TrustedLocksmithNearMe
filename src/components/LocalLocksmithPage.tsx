...
function heroText(service: MaServiceContent) {
  switch (service.slug) {
    case "emergency-locksmith": return "Locked out? See the price for the time you need help before you request it.";
    case "house-lockout": return "Locked out of your home? See the price for your service time before you request help.";
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        <section className="border-b border-line/60">
          <div className="mx-auto max-w-[1180px] px-6 py-10 sm:px-8 sm:py-12 lg:px-10">
            <div className="flex flex-wrap items-center gap-2 text-xs text-parchment-dim">
              <Link href={`/${city.slug}`} className="hover:text-parchment hover:underline">{city.name}</Link>
              <span>/</span>
              <span>{service.shortTitle}</span>
            </div>
            <div className="mt-5 font-mono text-[10px] uppercase tracking-[.14em] text-brass">{city.name}, Massachusetts</div>
            <h1 className="mt-4 max-w-3xl font-display text-[38px] font-medium leading-[.98] tracking-[-.035em] text-parchment sm:text-[44px] lg:text-[50px]">{service.shortTitle} in {city.name}</h1>
            <p className="mt-4 max-w-xl text-[15px] leading-6 text-parchment-dim">{heroText(service)}</p>
            <Link href={bookingHref} className="mt-6 inline-flex min-h-11 items-center justify-center rounded-full bg-brass px-6 py-2.5 text-sm font-semibold text-ink transition hover:brightness-110">Get my price</Link>
          </div>
        </section>
        ...
        <SimpleServiceLocalContext slug={city.slug} name={city.name} areas={city.areas} serviceLabel={service.shortTitle} />
        ...
        <section className="py-12 sm:py-14">
          <div className="mx-auto grid max-w-[1180px] gap-8 px-6 sm:px-8 lg:grid-cols-2 lg:px-10">
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-x-5 gap-y-3 text-sm">
                <Link href={`/${city.slug}`} className="font-semibold text-brass hover:underline">All {city.name} locksmith services →</Link>
