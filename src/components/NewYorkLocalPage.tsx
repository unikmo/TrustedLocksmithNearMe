...
const FEATURED_SERVICES: Array<{
...
}> = [
  {
    ...
    note: "Residential lockout pricing by service window",
    bookingHref: "/book/details?service_id=home_lockout_day",
  ...
  },
  {
    ...
    bookingHref: "/book/details?service_id=car_lockout_at_property",
  ...
  },
  {
    ...
    bookingHref: "/book/details?service_id=standard_rekey",
    priceSuffix: "standard total",
  },
  {
    ...
    bookingHref: "/book/details?service_id=standard_lock_change",
  ...
  },
  {
    ...
    bookingHref: "/book/details?service_id=smart_lock_install",
    priceSuffix: "labor · hardware separate",
  },
];
  ...
  return (
    <section className="border-b border-line/60 py-14 sm:py-16">
      <div className="mx-auto max-w-[1180px] px-6 sm:px-8 lg:px-10">
        {children.length > 0 && (
          <div>
            <div className="eyebrow">{area.kind === "city" ? "NYC boroughs" : `More in ${area.name}`}</div>
            <h2 className="mt-3 max-w-3xl font-display text-3xl tracking-[-.025em] text-parchment sm:text-4xl">Go closer to the actual service address.</h2>
            <div className="mt-7 grid gap-x-8 gap-y-3 border-y border-line/70 py-5 sm:grid-cols-2 lg:grid-cols-3">
              {children.map((child) => (
                <Link key={child.slug} href={`/${child.slug}`} className="group flex items-center justify-between gap-4 py-2 text-sm font-semibold text-parchment">
                  <span>{child.name}</span><span className="text-brass transition group-hover:translate-x-0.5" aria-hidden="true">→</span>
                </Link>
              ))}
            </div>
          </div>
        ...
        {upstateAndCounty.length > 0 && (
          <div className={children.length > 0 ? "mt-12" : ""}>
            <div className="eyebrow">New York State markets</div>
            <h2 className="mt-3 max-w-3xl font-display text-3xl tracking-[-.025em] text-parchment sm:text-4xl">Beyond New York City.</h2>
            <div className="mt-7 grid gap-x-8 gap-y-3 border-y border-line/70 py-5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {children.length === 0 && related.length > 0 && (
          <div>
            ...
            <div className="mt-5 flex flex-wrap gap-x-6 gap-y-3 border-y border-line/70 py-5">
              {related.map((item) => (
                <Link key={item.slug} href={`/${item.slug}`} className="text-sm font-semibold text-brass hover:underline">{item.name} →</Link>
              ))}
            </div>
          </div>
        )}
      ...
      <main className="flex-1">
          ...
          <div className="mx-auto max-w-[1180px] px-6 sm:px-8 lg:px-10">
            <div className="max-w-2xl">
              ...
              <p className="mt-4 text-sm leading-6 text-[#536e8a]">
                Dedicated local service pages are created only where the geography and service intent justify them. The published total still includes provider travel/service call for the stated scope.
              </p>
            </div>
            ...
            <div className="mt-9 divide-y divide-[#c7d9ec] border-y border-[#c7d9ec]">
              {FEATURED_SERVICES.map((service) => {
                ...
                if (!item) return null;
                const hasLocalPage = areaHasNyService(area.slug, service.slug);
                const href = hasLocalPage ? `/${area.slug}/${service.slug}` : service.bookingHref;
                return (
                  <Link key={service.slug} href={href} className="group grid gap-3 py-5 sm:grid-cols-[1fr_auto] sm:items-center">
                    <div>
                      ...
                      {hasLocalPage && <div className="mt-1 font-mono text-[10px] uppercase tracking-[.12em] text-[#7d6330]">Local {area.name} guide</div>}
                    </div>
        ...
        <section className="py-16 sm:py-20">
          <div className="mx-auto max-w-3xl px-6 text-center sm:px-8">
            ...
            <p className="mx-auto mt-4 max-w-xl text-sm leading-6 text-parchment-dim">No membership required for a one-off request. A specific provider is shown only after that provider actually accepts.</p>
